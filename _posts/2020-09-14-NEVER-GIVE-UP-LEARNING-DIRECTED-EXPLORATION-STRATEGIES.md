...
Embedding network $f$用于将observation映射成低维的向量，这个向量对应于可控制的状态,强调'可控'的原因是，对于状态自身会随时间改变的环境（作者举了一个交通状况的例子，即使什么都不做，也可以观测到大量不同的observation)，这种形式的状态探索和动作的探索性没有关系，也就不应该为该动作提供探索奖励为此，作者通过训练一个孪生网络来预测状态改变与动作的关系，以剔除与动作对环境的影响无关的因素。如图所示，两个孪生网络接受先后两个observation，它们的输出会拼接在一起，输入一个含一层隐藏层的全连接神经网络分类器$h$正式的表示:为对于一条经验$\\{x_t,a_t,x_{t+1}\\}$，分类器输出概率 $ p(a|x_t,x_{t+1})=h(f(x_t),f(x_{t+1}))$,
